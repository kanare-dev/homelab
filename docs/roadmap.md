...
- [x] 常用リモート開発環境を `vm-dev` から分離し、新VM `vm-code`（`192.168.11.22`, vmid 122, vm-dev 同等スペック）として切り出す
  ...
  - Terraform（`terraform.tfvars`）で VM 定義、Ansible ロール `code_tools` で git / mosh / gh / Node.js / Claude Code CLI / CloudCLI（`@cloudcli-ai/cloudcli`, siteboon/claudecodeui）/ code-server を再現可能にプロビジョニング
