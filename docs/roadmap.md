...
- [ ] Tailscale は SSH 等の管理アクセス・日常のリモートアクセス双方の主経路として継続利用する
...
- [ ] 常用リモート開発環境を `vm-dev` から分離し、新VM（`vm-code` 想定）として切り出す
  - 背景: `vm-dev` は「破壊検証用」と「Claude Code + モバイル remote control による外出先からの常用開発」を兼ねてしまっており、破壊検証中に常用環境を壊すリスクがあるため分離する
  - Claude Code / gh は現在 `vm-dev` に手動インストールしているため、新VMでは Ansible ロール化して再現可能にする
  - code-server（ブラウザ版 VS Code）も同居させ、CLI（Claude Code）とブラウザ編集を両方カバーする
  - アクセス経路は Tailscale のみに限定する（コード実行環境のため Cloudflare Tunnel 等での外部公開はしない）
  - メモリ的には現状 9.5GB/24GB 使用のため余裕あり（vm-dev 同等スペックで追加可能）
