# Day 0 / Day 1 / Day 2 手順

homelab の構築から運用までを 3 フェーズに分けて整理する。

現在は `8GB RAM` の Proxmox ホストを前提に、`vm-infra` と `vm-monitoring` を常時運用し、`vm-dev` は必要時だけ起動する最小構成を優先する。

## Day 0 — 計画・準備

Day 0 は「何を作るか」を決め、基盤を準備するフェーズ。

### Day 0 チェックリスト

- [x] ハードウェア調達・ラッキング
- [x] Proxmox VE インストール（`192.168.11.10`）
- [x] ネットワーク設計確定（IP 規約は `README.md` 参照）
- [x] SSH 鍵ペア生成
- [x] このリポジトリをクローン
- [x] Cloud-init テンプレート VM を作成（`terraform/proxmox/README.md` 参照）
- [x] Proxmox API トークン発行
- [x] `terraform.tfvars` を準備
- [x] DNS ドメイン（`lab.kanare.dev`）を決定
- [x] 秘密情報管理方針を決定（Ansible Vault を採用）
- [x] 初期構成は `vm-infra` + `vm-monitoring` + `vm-dev` に絞る

### Day 0 実行

```bash
# 1. Terraform で Core VM をプロビジョニング
cd terraform/proxmox
cp terraform.tfvars.example terraform.tfvars
vim terraform.tfvars
terraform init
terraform plan
terraform apply
```

```bash
# 2. まずは常時運用する VM に SSH できることを確認
ssh ubuntu@192.168.11.11
ssh ubuntu@192.168.11.13

# 3. vm-dev を作った場合だけ確認
ssh ubuntu@192.168.11.21
```

## Day 1 — 構築・デプロイ

Day 1 は VM の初期セットアップとサービスデプロイを行うフェーズ。

### 優先順

1. `vm-infra` を先に整える
2. `vm-monitoring` をコード経由で再現する
3. `vm-dev` を検証用に用意する
4. `vm-apps` は必要なアプリが決まってから追加する

### Day 1 チェックリスト

- [x] Ansible inventory を環境に合わせて編集
- [x] 常時起動する VM に共通セットアップを適用（パッケージ, SSH 硬化, UFW, Docker）
- [x] `vm-infra`: CoreDNS デプロイ
- [x] `vm-infra`: Caddy を Docker Compose で構築し、Cloudflare DNS-01 チャレンジで Let's Encrypt 証明書を取得
- [x] `vm-monitoring`: Prometheus + Grafana デプロイ
- [x] 常時起動する VM に `node_exporter` デプロイ
- [x] DNS 名前解決テスト
- [x] Grafana ダッシュボード設定
- [x] Reverse Proxy 経由のアクセス確認
- [ ] `vm-dev` を必要に応じて作成し、検証用に使える状態にする

### Day 1 実行

```bash
# 1. Ansible で vm-infra / vm-monitoring をセットアップ
cd ansible
ansible-playbook playbooks/site.yml
```

```bash
# 2. DNS 動作確認（vm-infra 上で）
dig @192.168.11.11 grafana.lab.kanare.dev
dig @192.168.11.11 prometheus.lab.kanare.dev
```

```bash
# 3. Monitoring 動作確認
# ブラウザで http://192.168.11.13:3000 にアクセス
# ブラウザで http://192.168.11.13:9090 にアクセス
# Grafana にログインして datasource と dashboard を確認
```

```bash
# 4. Reverse Proxy 動作確認
# ブラウザで https://grafana.lab.kanare.dev にアクセス
# ブラウザで https://prometheus.lab.kanare.dev にアクセス
```

```bash
# 5. vm-dev が必要になったら追加して検証用に使う
ssh ubuntu@192.168.11.21
```

### 推奨ダッシュボード（Grafana）

| ID | 名前 | 用途 |
| --- | --- | --- |
| 1860 | Node Exporter Full | VM のシステムメトリクス |
| 3662 | Prometheus 2.0 Overview | Prometheus 自体の状態 |

## Day 2 — 運用・改善

Day 2 は継続的な運用と改善のフェーズ。

### 定期作業

| 頻度 | 作業 | コマンド |
| --- | --- | --- |
| 毎日 | Grafana でメトリクス確認 | ブラウザ |
| 週次 | 常時起動 VM の `apt upgrade` | `ansible all -m apt -a "upgrade=yes update_cache=yes"` |
| 月次 | Docker イメージ更新 | `docker compose pull && docker compose up -d` |
| 月次 | Terraform state 確認 | `terraform plan` |
| 随時 | バックアップ検証 | `docs/runbook.md` 参照 |

### 改善ロードマップ

- [x] Tailscale を vm-infra にサブネットルーターとして導入（外出先からホームラボへのリモートアクセス）
- [ ] DNS を AdGuard Home + Unbound に移行（CoreDNS を置き換え、広告ブロック・クエリログ・プライバシー向上）
- [ ] Cloudflare Tunnel + Split-Horizon DNS で外部からも同一 FQDN でアクセスできるようにする
- [ ] Alertmanager 導入（Slack / Discord 通知）
- [ ] blackbox_exporter で外形監視を追加
- [ ] `vm-apps` を有効化してアプリを載せる
- [ ] Home Assistant 導入
- [ ] ログ収集（Loki + Promtail）
- [ ] バックアップ自動化（Proxmox Backup Server or restic）
- [ ] VM テンプレートの Packer 化
- [ ] CI/CD パイプライン（GitHub Actions で lint + plan）
- [ ] メモリ増設後に Kubernetes クラスタ（k3s）を検討
- [ ] UPS 導入と自動シャットダウン
