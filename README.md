...
現在は `8GB RAM` の ThinkCentre Tiny を前提に、無理なく常時運用できる構成を維持している。`vm-infra`・`vm-monitoring`・`vm-apps` が常時稼働し、`vm-dev` は必要なときに自由に壊して試せる検証用 VM とする。
...
│  │  .11  1GB    │  │  .13  2GB     │  │  .20  1GB    │           │
│  │  CoreDNS     │  │  Prometheus   │  │  Homepage    │           │
│  │  Caddy       │  │  Grafana      │  │              │           │
...
| ネットワーク | CoreDNS, Caddy, Tailscale | 内部 DNS、reverse proxy、VPN（サブネットルーター） |
...
| `vm-infra` | `111` | `192.168.11.11` | 1GB | 常時起動 | インフラ基盤 | CoreDNS, Caddy, Tailscale |
...
| ESP32 | [https://192.168.11.100](https://192.168.11.100) | - | ESP32簡易表示 |
